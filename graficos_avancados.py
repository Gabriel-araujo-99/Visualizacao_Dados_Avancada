...
def criar_grafico(df):
    # Histograma
    fig1 = px.histogram(df, x='salario', nbins=30, title='Distruibuição de Salários')
    ...
    fig4.update_layout(
        title='Salário por Idade e Aréa de Atuação por cada Nível de Educação',
        xaxis_title='Idade',
        yaxis_title='Salário'
    )
...
def criar_app(df):
    ...
    fig1, fig2, fig3, fig4, fig5, fig6 = criar_grafico(df)
